...
function VaccinationsList({ patientId }) {
  ...
  const fetchVaccinations = React.useCallback(async () => {
    try {
      ...
      if (response.data && response.data.vaccinations) {
        // Handle both old format (vaccinationDetails/futureVaccinations) and new format (completedVaccinations/futureVaccinations)
        const processedVaccinations = response.data.vaccinations.map(vac => ({
          ...vac,
          completedVaccinations: vac.completedVaccinations || vac.vaccinationDetails || [],
